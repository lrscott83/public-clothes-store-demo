...
- [ ] 10.4 Retire the pre-reshape `CompanyUser` from `apps/api-idp/src/users/users.service.ts` (+ spec, `users.module.ts`). **Added after 6.5's audit** — 6.5 predicted the remainder would be the api-common guards plus the saga, and it undercounted: `UsersService` holds the old shape and no task named it. While here, fix the two long-standing typecheck errors this file's spec carries (`users.service.spec.ts:30` missing `createdByCompanyUserId`, and its twin now at `auth.service.spec.ts:73` — line number shifted by this batch's edits, same pre-existing gap, count still exactly 2, confirmed by isolated `apps/api-idp` typecheck run after this batch). **Also still open, not touched by this batch's 10.2 resolution**: `AuthService`'s `companyUserRepository`/`ICompanyUserRepository` injection (used by `resolveRole` for login/refresh bitmask resolution) still binds the pre-reshape `CompanyUser` shape — only signup's ASSIGNMENT WRITE was removed, not this read path. Whether that read path is itself in 10.4's scope or needs its own task is an open question for whoever picks this up next.
