...
- [x] 10.2 [GREEN] `apps/api-idp/src/company/create-company.saga.ts` — 6 steps with reverse-order compensation (§D7), step 6 (catalog copy, Phase 9) AWAITED, both tenant-Membership-pairing writes (steps 4/5) staying consistent with Phase 8's precedent. `ICompanyRepository` gained `create`/`setSchemaName`/`delete` and `IMembershipRepository` gained `delete` (needed by the saga's forward steps and compensation; not itemized in design.md's file map but structurally required by D7). `schemaNameFor`/`assertSchemaName` now exported from `@store-mgmt/infra-db`'s public index — the saga is their first consumer outside the package.
  - **STOP-AND-REPORT, per this task's own instruction — "Replace `AuthService.signup`'s `resolveSoleCompany` with explicit `Membership` lookup" NOT done.** `auth.service.ts`/`auth.service.spec.ts` are UNTOUCHED beyond a mechanical 3-stub addition to `buildCompanyRepoMock()` (kept `ICompanyRepository`'s now-larger interface type-compatible; zero behavior/assertion change). design.md D7's one-line instruction is genuinely underspecified: the saga's own 6 steps take an EXISTING `ownerId`, so a brand-new `signup()` registrant has no Membership to "look up" yet, and no task anywhere wires an HTTP surface that calls this saga — there is no evidence for what "explicit Membership lookup" should concretely replace `resolveSoleCompany` with (candidates considered and rejected for lack of textual support: requiring a companyId in `SignupDto`; filtering `companyRepository.list()` to `schemaName !== null`; filtering to companies with an existing Membership). Rewriting live signup/authorization logic on a guess is a bigger risk than leaving `resolveSoleCompany` as-is for one more batch. Needs an explicit owner decision before this sub-clause is implemented — likely as its own follow-up task rather than folded silently into 10.2.
