...
**Flagged during 7.1 (not fixed here, out of this phase's file scope):** `IMembershipRepository.findActiveByUserId` (`PrismaMembershipRepository`, Phase 3) is a `findFirst` with no `orderBy` and no distinctness check. The master `Membership` schema only enforces uniqueness on `(userId, companyId)` — a user genuinely CAN hold ACTIVE `Membership` rows in more than one company (that is the point of multi-tenancy). If such a user calls without `X-Company-Id`, `TenantContextGuard` resolves whichever row Postgres returns first, silently, rather than requiring the header to disambiguate. Neither `spec.md`'s "Tenant Resolution Guard Chain" requirement nor `tasks.md` 7.1 asked for ambiguity detection, and the guard cannot implement it without a repository method the current port (`IMembershipRepository`, shipped Phase 1) doesn't expose (e.g. `listActiveByUserId`). Left as-is per this phase's scope lock — worth a design decision (extend the port, or accept "first active wins" as intended UX for the common single-company case) before Phase 12/13's e2e coverage assumes single-company users only.
