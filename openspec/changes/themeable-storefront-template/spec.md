# Spec: Themeable Storefront Template

Change: `themeable-storefront-template` | Project: `public-clothes-store-demo` | Artifact store: hybrid

Locked scope for this spec: package `@store-mgmt/storefront` at `templates/packages/storefront` (subpath exports `./theme`, `./catalog`, `./config`); reference app `templates/apps/static-store`. Legacy `src/` is read-only reference and MUST NOT be modified by any requirement below.
