# Delta for salesops-delivery (AMENDMENT, not append)

**Merge target**: `openspec/specs/salesops-delivery/spec.md` (promoted). This document
AMENDS four of that spec's requirements in place and ADDS four more, closing the gap
between the promoted spec and the `judgment-day` remediation already shipped on
`salesops-delivery`. It does not touch any requirement not listed below — everything else
in the promoted spec (Carrier-Warehouse Coverage join table, Sales Remains the Sole Owner
of `Order.status`, `POST /orders/:id/deliver` Is Unrestricted, A Delivered Order Never
Leaves an Open Assignment Behind, Coverage Is Advisory) still holds exactly as written.
