...
- [x] 5.1 `apps/api-salesops/src/app.module.ts`: wire `USER_REPOSITORY → PrismaUserRepository`, Passport `JwtStrategy` (from `@store-mgmt/api-common`), global or per-module `JwtAuthGuard`. DONE via a NEW `src/auth/auth.module.ts` (mirrors `apps/api-idp`'s `AuthModule` precedent: `JwtStrategy` + `USER_REPOSITORY→PrismaUserRepository` bound once, eagerly imported by `AppModule` — usable from any controller via `JwtAuthGuard`/`AuthGuard('jwt')` without per-module re-import, since Passport strategy registration is a bootstrap side effect, not DI scoping). Per-controller `@UseGuards(JwtAuthGuard, RolesGuard)` chosen over a single global guard (task explicitly allows either) so `HealthController` stays an unguarded infra liveness probe.
- [x] 5.2 [RED] `stock.controller.spec.ts` / `ventas.controller.spec.ts` / other guarded controllers (extend, additive cases): unauthenticated request → 401; authenticated user lacking the required role → 403; user holding the role (or `admin`) → 200. DONE for ALL 7 business controllers (category/product/warehouse/currency/customer/stock/ventas) — each spec confirmed 500 (`AuthGuard('jwt')` throwing "Unknown authentication strategy") before `overrideJwtAuth`/`RolesGuard` provider wiring landed, i.e. real RED before GREEN.
- [x] 5.3 [GREEN] Apply `@UseGuards(JwtAuthGuard, RolesGuard)` + `@Roles(...)` to existing endpoints. The tasks/design artifacts left the exact per-endpoint matrix as "TBD" (tasks.md line 121 itself says `operador_gestores`: TBD); the orchestrator supplied a locked baseline matrix, applied verbatim: catalog (category/product) read=any-auth, write=owner/admin; warehouse read=any-auth, write=owner/admin; rate(currency) read=any-auth, write=owner/admin; customer read+write=owner/admin/sales_operator (class-level, uniform); stock read+move=owner/admin/warehouse_operator + warehouse-operator scoped to own `warehouseId` (403 on mismatch); orders create/update/confirm/cancel=owner/admin/sales_operator, list/findById=+warehouse_operator scoped to own warehouse, deliver=owner/admin/warehouse_operator (NOT sales_operator) scoped to own warehouse. `WAREHOUSE_OPERATOR_REPOSITORY` bound in `StockModule`/`VentasModule`; scope-check helpers live in the controllers (mirrors `HealthController`'s existing precedent of direct repository injection in a controller for an infra/authz concern, not business logic).
- [x] 5.4 [RED] `customer.controller.spec.ts` / `customer.service.spec.ts` (extend): `create` now requires an existing `userId`. ALREADY DONE in a prior Phase 2 apply run (verified: `CreateCustomerDto.userId` required, `CustomerUserNotFoundError`/`DuplicateCustomerUserError` domain errors + 400/409 mapping all pre-existing and covered by tests) — no new work needed here, only the guard wiring (5.2/5.3) on top.
- [x] 5.5 [GREEN] `customer.service.ts`/`customer.controller.ts`/`create-customer.dto.ts` — confirmed already GREEN from the prior Phase 2 work; re-verified passing in this run's full suite.
- [x] 5.6 [RED] `apps/api-salesops/test/customer.e2e-spec.ts` (extend): create-Customer-without-User → 400; create-Customer-with-User → 201, `userId` round-trips. ALREADY DONE in prior Phase 2 apply run; this run additionally added `Authorization: Bearer` (owner/admin/sales_operator caller) to every request (5.8) plus new 401/403 RolesGuard cases.
- [x] 5.7 [GREEN] Confirmed 5.6 passes against 5.4/5.5 wiring — 14/14 green including the new RolesGuard cases.
- [x] 5.8 [RED→GREEN] Added `apps/api-salesops/test/support/auth-e2e-helper.ts` (`createAuthedUser`/`createAuthedWarehouseOperator`/`authHeader`) — mints a REAL `User` row via Prisma + signs a REAL access JWT directly with the shared `JWT_CONFIG.secret` from `@store-mgmt/api-common` (no round-trip to `api-idp`; this app "verifies tokens locally" per spec, so local signing with the same secret is a faithful e2e substitute for login). Attached `Authorization: Bearer <token>` to every request in `category`/`product`/`warehouse`/`customer`/`ventas` e2e-specs (roles matched to each endpoint's matrix); added 401 (no token) + 403 (wrong role) cases to each, plus dedicated `warehouse_operator` scope tests (deliver own-vs-other-warehouse, list filtering) in `ventas.e2e-spec.ts`. `health.e2e-spec.ts` untouched (unguarded by design).
- [x] 5.9 Ran `pnpm --filter @store-mgmt/api-salesops test && test:e2e` full-green — 180/180 unit (15 suites) + 50/50 e2e (6 suites, real Postgres, rebuilt `domain`+`infra-db`+`api-common` dist first); `typecheck`/`build`/`lint --max-warnings 0` all exit 0.
