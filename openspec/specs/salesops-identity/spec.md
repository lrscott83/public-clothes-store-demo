# Spec — salesops-identity (amendment for multi-tenant-by-schema)
...
This spec now exists in main `openspec/specs/`.

## Purpose

Define authentication and authorization for multi-tenant topology: JWT validation at the 
master level, tenant resolution via guard chain with explicit Membership gates, and role 
resolution at tenant context time.
...
email verification, and the `Invitation`/invite-accept flow.
