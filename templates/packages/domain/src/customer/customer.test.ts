...
const COMPANY_USER_ID = 'company-user-uuid-1';

describe('createCustomer — invariants', () => {
  it('rejects an empty fullName', () => {
    expect(() => createCustomer({ fullName: '', companyUserId: COMPANY_USER_ID })).toThrow(
  ...
  it('rejects a missing companyUserId', () => {
    expect(() => createCustomer({ fullName: 'Ana Torres', companyUserId: '' })).toThrow(
      InvalidCustomerError,
    );
  });

  it('rejects a whitespace-only companyUserId', () => {
    expect(() => createCustomer({ fullName: 'Ana Torres', companyUserId: '   ' })).toThrow(
      InvalidCustomerError,
    );
  });
  ...
  it('accepts a valid fullName and defaults active=true', () => {
    const customer = createCustomer({ fullName: 'Ana Torres', companyUserId: COMPANY_USER_ID });
    expect(customer.fullName).toBe('Ana Torres');
    expect(customer.companyUserId).toBe(COMPANY_USER_ID);
