...
/** Config-driven storefront footer. `linkGroups`/`contact`/`social` are all optional and degrade gracefully; only `copyright` is required. */
        ...
        {(hasLinkGroups || footer.contact || footer.social) && (
          <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-4 mb-8">
            ...
            {(footer.contact || footer.social) && (
              <div>
                <h3 className="text-sm font-semibold text-text mb-3">Contacto</h3>
                {footer.contact && <p className="text-sm text-text-muted">{footer.contact}</p>}
                {footer.social && footer.social.length > 0 && (
                  <ul className="flex gap-4 mt-3">
                    {footer.social.map((social) => (
                      <li key={social.url}>
                        <a href={social.url} className="text-sm text-text-muted hover:text-text">
                          {social.label}
                        </a>
                      </li>
                    ))}
