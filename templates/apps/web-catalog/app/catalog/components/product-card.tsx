...
export function ProductCard({ item, locale }: ProductCardProps) {
  return (
    <div className="group relative rounded-lg shadow-card overflow-hidden bg-surface transition-transform duration-300 hover:scale-[1.02]">
      ...
      <div className="p-3">
        <h3 className="text-xs font-semibold text-text leading-tight truncate" title={item.name}>
          {item.name}
        </h3>
        ...
        <div className="mt-1.5 flex items-center gap-1.5">
          ...
          {item.isOffer && (
            <span data-testid="product-card-original-price" className="text-xs line-through text-text-muted">
