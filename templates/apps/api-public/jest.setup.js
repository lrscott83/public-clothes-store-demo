...
// database BEFORE any test file (and therefore any PrismaMasterService/
// TenantPrismaFactory) is constructed — identical to
// `apps/api-salesops/jest.setup.js` (this app reads the same master schema
// via `PrismaCompanyRepository`, so it must be pointed at the same test
// database). Test suites must NEVER touch the dev `store_mgmt` database.
const fs = require('fs');
