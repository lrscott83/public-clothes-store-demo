 ...
 * Warehouse W1), here protecting financial data.
 */
describe('Product (e2e)', () => {
  ...
  it('rejects clearing price to zero on update -> 400, original price untouched', async () => {
    const created = await request(app.getHttpServer()).post('/products').send(validProduct());
    ...
    expect(response.status).toBe(400);
    const found = await request(app.getHttpServer()).get(`/products/${created.body.id}`);
